...
# --- INICIALIZACIÓN DE CONTROLES (XBOX) ---
...
class Game:
    ...
    def draw(self):
        ...
        minutos = self.time //3600
        segundos = (self.time // 60) % 60
        tiempo_texto = f"{minutos:02}:{segundos:02}"
        screen.draw.text(tiempo_texto, (20,20), color= "pink", fontsize=30, shadow=(1,1))
...
class Controls:
    def __init__(self, player_num):
        ...
        # Intentamos asignar el control (0 para J1, 1 para J2 si hay otro control)
        try:
            if pygame.joystick.get_count() > self.player_num:
                self.joy = pygame.joystick.Joystick(self.player_num)
                self.joy.init()
            else:
                self.joy = None
        except:
    ...
    def move(self, speed):
        dx, dy = 0, 0
        if self.joy:
            # Stick izquierdo: Eje 0 (Horizontal) y Eje 1 (Vertical)
            joy_x = self.joy.get_axis(0)
            joy_y = self.joy.get_axis(1)
            
            # Umbral de 0.2 para que el jugador no camine solo
            if abs(joy_x) > 0.2: dx = joy_x
            if abs(joy_y) > 0.2: dy = joy_y
        else:
            # Si no hay control, sigue funcionando el teclado
            if self.player_num == 0:
                if keyboard[keys.LEFT]: dx = -1
                elif keyboard[keys.RIGHT]: dx = 1
                if keyboard[keys.UP]: dy = -1
                elif keyboard[keys.DOWN]: dy = 1
        return Vector2(dx, dy) * speed

    def shoot(self):
        # En Xbox, el botón 0 es la 'A'
        if self.joy:
            return self.joy.get_button(0)
        
        key = keys.SPACE if self.player_num == 0 else keys.LSHIFT
...
def update():
    ...
    if state == State.MENU:
        # Revisamos si se presionó Espacio O el botón A del control 0
        joy_pressed = False
        if pygame.joystick.get_count() > 0:
            joy_pressed = pygame.joystick.Joystick(0).get_button(0)

        if key_just_pressed(keys.SPACE) or joy_pressed:
